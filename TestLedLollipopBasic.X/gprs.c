...
bool gprsCCIDParser(void);
bool gprsCCIDParser(void);
bool gprsInternetAccessParser(void);
bool gprsCGSNParser(void);
...
#define GPRS_AT_CMD_GET_REG_STAT_INDEX      10
...
const GPRS_AT_CMD_STRUCT_T gprsAtCommands[]={
    ...
    {GPRS_AT_CMD_GET_ICID_INDEX,"AT#CCID",gprsCCIDParser,2},
    {GPRS_AT_CMD_GET_IMEI_INDEX,"AT#CGSN",gprsCGSNParser,2},
    {GPRS_AT_CMD_SOCKET_DIAL_INDEX,"AT#SD=1,1,",gprsConnectParser,2},
    {GPRS_AT_CMD_GET_REG_STAT_INDEX,"AT+CGREG?",gprsInternetAccessParser,2},
    {GPRS_COMMAND_LIST_END,"",NULL,0}
};
...
bool gprsCCIDParser(void){
    const char ccidStr[] = "+CCID:";
    const char cChar=grpsRxBuffer;
    if (strstr(cChar, ccidStr)== NULL)
        return(false);
        char * ptr=strchr(gprsRxBuffer,(int)':');
    if(ptr==NULL)
        return false;
    strcpy(sysVarCCID,ptr+1);
    return true;
}
bool gprsInternetAccessParser(void){
...
bool gprsCGSNParser(void){
    if (strstr(gprsRxBuffer, "#CGSN:")==NULL)
        return false;
        char * ptr=strchr(gprsRxBuffer,':');
    if(ptr==NULL)
        return false;
    strcpy(sysVarIMEI,ptr+1);
    return true;
}
bool gprsConnectParser(void){
...
void gprsCommandSelector(GPRS_AT_CMD_T gprsCommand){
    ...
    eeprom char *epromPtr;
    
    gprsTimeOutTimer = gprsCommandPtr->atCmdTimeOut;
    strcpy(gprsTxBuffer,gprsCommandPtr->atCmdStr);
    switch(gprsCommand){
        case GPRS_AT_CMD_CTXT_CONFIG_INDEX:
            epromPtr = sysVarAPN;
            strcat(gprsTxBuffer,epromPtr);
            break;
        case GPRS_AT_CMD_SOCKET_DIAL_INDEX:
            epromPtr=sysVarDestPORT;           
            strcat(gprsTxBuffer,epromPtr);
            strcat(gprsTxBuffer,",");
            epromPtr=sysVarDestADDRESS;
            strcat(gprsTxBuffer,epromPtr);
            break;
        case GPRS_AT_CMD_SEND_MSG_INDEX:
            sprintf(gprsTxBuffer,"%4.4X",sysVarUNIXDayTime);
            break;
        case GPRS_AT_CMD_SCAPE_SEQ_INDEX:
            return;
    }
    strcat(gprsCommandPtr,gprsCOMMAND_TAIL_STR);
}
/*
 LOW LEVEL
 */
...
taskResult_t gprsTask(void){
    ...
    const uint8_t gprsSendMsgCommandROMList[] = {
        ...
        GPRS_AT_CMD_GET_REG_STAT_INDEX,
        GPRS_AT_CMD_SOCKET_DIAL_INDEX,
        GPRS_AT_CMD_SEND_MSG_INDEX,
        GPRS_AT_CMD_SCAPE_SEQ_INDEX,
        GPRS_COMMAND_LIST_END
    };
    static const GPRS_AT_CMD_STRUCT_T * gprsRomCommandPtr;
    static const char *gprsFlashCommandPtr;
    ...
    switch(gprsTaskState){
        ...
        case TASK_RESET_TASK:
            ...
            gprsFlashCommandPtr = gprsInitCommandsROMList;
// TODO           gprsTransactionAttemps = 
            while(gprsFlashCommandPtr[0] != GPRS_COMMAND_LIST_END)
            {
                gprsTransactionResult = gprsCommManager(gprsFlashCommandPtr[0]);
                ...
                if(gprsTransactionResult == TASK_DATA_READY){
                    ...
                    {
                        gprsFlashCommandPtr++;
                        gprsRomCommandPtr = &gprsAtCommands[gprsFlashCommandPtr[0]];
                ...
                if(gprsTransactionResult == TASK_NO_SENSOR_FOUND){                   
                    if(gprsRomCommandPtr->index == GPRS_AT_CMD_ATENTION_INDEX){
                        break;
                    }
                    gprsTaskPowerDown();
                }
        ...
        case TASK_READY:
            if(gprsScheduledMsgTaskFlag){
                ...
                gprsCommManagerState = TASK_SEND_COMMAND_SM;
                gprsFlashCommandPtr = gprsSendMsgCommandROMList;
                return(TASK_ON_COURSE);
            }
            //VERIFICAR
        ...
        case TASK_WAITING_DATA:
            while(gprsFlashCommandPtr[0] != GPRS_COMMAND_LIST_END){
                ...
                if(gprsTransactionResult == TASK_DATA_READY){
                    if(gprsRomCommandPtr->gprsParserPtr()){
                        gprsRomCommandPtr = &gprsAtCommands[gprsFlashCommandPtr[0]];
                        gprsTransactionAttemps = gprsRomCommandPtr->cmdRepeatAttemps;
                        continue;
                    }
            ...
            else if(gprsScheduledGetEpoFlag){
                gprsScheduledGetEpoFlag = false;     
            }
            gprsTaskState = TASK_READY;
            return(TASK_ON_COURSE);
...
void gprsTaskNewTask(GPRS_TASK_COMMANDS_T command){
    switch(command){
       ...
       //TODO     gprsScheduledMsgStaTaskFlag = false;
            break;
